...
export default function ChatPage({ onNavigate }) {
  ...
  return <main className="flex h-[100dvh] overflow-hidden bg-canvas">
    <ConversationSidebar conversations={conversations} activeId={activeConversationId} loading={historyLoading} error={historyError} open={sidebarOpen} disabled={sidebarDisabled} onClose={() => setSidebarOpen(false)} onNew={createNewConversation} onSelect={openConversation} onDelete={deleteConversation} />
    ...
    <section className="flex min-w-0 flex-1 flex-col overflow-hidden">
      ...
      <div className="min-h-0 flex-1 overflow-y-auto">
        {conversationLoading ? <div className="grid h-full place-items-center"><div className="text-center"><LoaderCircle className="mx-auto animate-spin text-teal" /><p className="mt-3 text-xs text-muted">Loading conversation…</p></div></div> : messages.length ? <MessageList messages={messages} isTyping={isTyping} /> : <section className="relative flex h-full min-h-[390px] flex-col items-center justify-center overflow-hidden px-5 pb-4 pt-6 text-center sm:px-8">
