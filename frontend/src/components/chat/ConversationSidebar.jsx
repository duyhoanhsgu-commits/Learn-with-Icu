...
export default function ConversationSidebar({ conversations, activeId, loading, error, open, disabled, desktopWidth = 286, onClose, onNew, onSelect, onDelete, onPersonalize }) {
  ...
  return <>
    ...
    <aside style={{ '--conversation-sidebar-width': `${desktopWidth}px` }} className={`conversation-sidebar-responsive fixed bottom-3 left-3 top-3 z-50 flex w-[min(286px,calc(100vw-24px))] shrink-0 flex-col rounded-[18px] border border-line bg-white px-4 py-5 text-ink shadow-[0_12px_36px_rgba(15,23,42,.10)] transition-transform duration-200 lg:static lg:z-auto lg:h-full lg:translate-x-0 lg:shadow-[var(--shadow-sm)] ${open ? 'translate-x-0' : '-translate-x-[calc(100%+24px)]'}`}>
