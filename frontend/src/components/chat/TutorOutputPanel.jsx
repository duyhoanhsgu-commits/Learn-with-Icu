...
export default function TutorOutputPanel({ messages, width, mobileOpen, onCloseMobile, onCollapse }) {
  ...
  return <>
    ...
    <aside style={{ '--tutor-output-width': `${width}px` }} className={`fixed bottom-3 right-3 top-3 z-50 flex w-[min(420px,calc(100vw-24px))] flex-col overflow-hidden rounded-[18px] border border-line bg-white text-ink shadow-[0_12px_36px_rgba(15,23,42,.10)] transition-transform xl:static xl:z-auto xl:h-full xl:w-[var(--tutor-output-width)] xl:translate-x-0 xl:shadow-[var(--shadow-sm)] ${mobileOpen ? 'translate-x-0' : 'translate-x-[calc(100%+24px)]'}`}>
