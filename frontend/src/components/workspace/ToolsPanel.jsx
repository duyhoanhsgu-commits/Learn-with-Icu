  ...
  return <aside className="flex h-full min-w-0 flex-col border-l border-line bg-white">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto border-t border-line bg-canvas/70 p-4 sm:p-5">
      <section className="rounded-[18px] border border-line bg-white p-4 shadow-[0_8px_25px_rgba(18,33,59,.05)] sm:p-5">
        ...
        <button disabled={disabled || loading || !currentPrompt.trim()} onClick={generate} className="icu-action-gradient mt-3 flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-[11px] font-bold text-white shadow-[0_8px_20px_rgba(52,133,245,.2)] transition hover:-translate-y-0.5 disabled:translate-y-0 disabled:cursor-not-allowed disabled:bg-none disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none">{loading && <LoaderCircle size={14} className="animate-spin" />}{loading ? 'Working…' : selected.actionLabel}</button>
      </section>
