...
export default function ToolsPanel({ disabled, spaceId, onCollapse }) {
  ...
  return <aside className="flex h-full min-w-0 flex-col overflow-hidden rounded-[18px] border border-line bg-white shadow-[var(--shadow-sm)]">
    ...
    <div className="grid gap-2 px-4 pb-4 sm:px-5">
      {tools.map((tool) => {
        ...
        return <button key={tool.id} onClick={() => setActive(tool.id)} className={`flex min-w-0 items-center gap-3 rounded-[14px] border px-3 py-2.5 text-left transition ${selectedTool ? 'border-brandblue/40 bg-brandblue/[.06]' : 'border-line bg-white hover:border-brandblue/25 hover:bg-[#fffaf6]'}`}><span className={`grid h-9 w-9 shrink-0 place-items-center rounded-[10px] ${selectedTool ? 'bg-brandblue/10 text-brandblue' : 'bg-[#fff4ea] text-brandblue'}`}><Icon size={16} /></span><span className="min-w-0"><span className={`block truncate text-[10px] font-semibold ${selectedTool ? 'text-ink' : 'text-slate-700'}`}>{tool.name}</span><span className="mt-0.5 block truncate text-[9px] text-muted">{tool.id === 'quiz' ? 'Test your knowledge' : tool.id === 'mindmap' ? 'Visualize concepts' : 'Study & review'}</span></span></button>
      })}
    </div>
    ...
    <div className="min-h-0 flex-1 overflow-y-auto border-t border-line bg-canvas/70 p-4 sm:p-5">
      <section className="rounded-[18px] border border-line bg-white p-4 shadow-[0_8px_25px_rgba(18,33,59,.05)] sm:p-5">
        ...
        <button disabled={disabled || loading || !currentPrompt.trim()} onClick={generate} className="icu-primary-action mt-3 flex w-full items-center justify-center gap-2 rounded-xl px-4 py-3 text-[11px] font-semibold text-white transition disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400 disabled:shadow-none">{loading && <LoaderCircle size={14} className="animate-spin" />}{loading ? 'Working…' : selected.actionLabel}</button>
      </section>
