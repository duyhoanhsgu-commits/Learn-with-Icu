  ...
  return <section className="tool-page fixed inset-0 z-[100] flex h-[100dvh] flex-col bg-canvas">
      ...
      <div className="mx-auto w-full max-w-[760px] px-5 py-6 sm:px-6 sm:py-10">
        <button onClick={() => setFlipped((value) => !value)} aria-pressed={flipped} aria-label={flipped ? 'Show flashcard question' : 'Reveal flashcard answer'} className="flashcard-scene block h-[min(440px,52vh)] min-h-[320px] w-full rounded-[22px] text-center">
          <span className={`flashcard-inner block h-full w-full ${flipped ? 'is-flipped' : ''}`}>
            ...
            <span aria-hidden={!flipped} className="flashcard-face flashcard-back absolute inset-0 flex flex-col items-center justify-center rounded-[20px] border border-teal/25 bg-[#fffaf6] p-7 sm:p-12"><span className="text-[9px] font-bold uppercase tracking-[.2em] text-teal">Answer</span><span className="mt-6 max-w-xl text-base font-medium leading-7 text-ink sm:text-xl sm:leading-8">{card.back}</span><span className="mt-9 flex items-center gap-1.5 text-[10px] text-muted"><RotateCcw size={12} />Click card or press Space to flip</span></span>
          </span>
        </button>
      </div>
    </main>
    ...
    <footer className="shrink-0 border-t border-line bg-white/95 backdrop-blur">
      <div className="mx-auto flex max-w-[760px] items-center gap-3 px-4 py-3 sm:px-6 sm:py-4">
        ...
        <button onClick={() => setFlipped((value) => !value)} className="h-11 flex-1 rounded-xl border border-brandblue bg-white px-5 text-xs font-semibold text-brandblue transition hover:bg-brandblue/[.06]">{flipped ? 'Show question' : 'Reveal answer'}</button>
