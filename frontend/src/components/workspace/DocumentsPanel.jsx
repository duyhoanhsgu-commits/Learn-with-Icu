...
export default function DocumentsPanel({ spaces, activeSpace, onSelectSpace, onCreateSpace, onUpload, onDeleteFile, selectedFile, onSelectFile, sourceTarget, onAsk, onCollapse }) {
  ...
  return <aside id="workspace-spaces-section" tabIndex={-1} className="flex h-full min-h-0 min-w-0 flex-col overflow-hidden bg-midnight px-4 py-5 text-white sm:px-5">
    ...
    <div id="workspace-documents-section" tabIndex={-1} className="mt-6 flex shrink-0 items-center justify-between border-b border-white/[0.08] pb-3"><div className="flex items-center gap-2"><FileText size={14} className="text-teal" /><p className="text-[10px] font-bold uppercase tracking-[.18em] text-slate-300">Documents</p><span className="rounded-full bg-white/[0.07] px-2 py-0.5 text-[9px] text-slate-400">{activeSpace?.files.length || 0}</span></div><button type="button" title="Document filters" className="rounded-lg p-1.5 text-slate-500 hover:bg-white/[0.06] hover:text-slate-200"><ListFilter size={14} /></button></div>

    <div role="region" aria-label="Documents in this learning space" tabIndex={0} className="document-list-scroll mt-3 min-h-0 flex-1 touch-pan-y space-y-2 overflow-y-auto overscroll-contain pb-2 pr-1 outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-teal/50">{activeSpace?.files.map((file) => <FileItem key={file.id} file={file} active={false} onClick={() => file.persisted && onSelectFile(file.id)} onDelete={file.persisted ? onDeleteFile : undefined} />)}{activeSpace && !activeSpace.files.length && <div className="grid h-full min-h-32 place-items-center px-5 text-center"><div><FileText size={25} className="mx-auto text-slate-600" /><p className="mt-3 text-xs font-semibold text-slate-300">No documents yet</p><p className="mt-1 text-[10px] leading-4 text-slate-500">Upload your first learning material below.</p></div></div>}</div>
